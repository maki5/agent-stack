...
- `profile.agents` — which developer agents exist for this project
...
Developer agents are listed in `profile.agents` in `opencode.json`. Delegate fix implementation to whichever developer agents own the affected layers.
...
Read `profile.agents` to determine which developer agents own the affected layers. Delegate to each:
...
9. Read `profile.agents` to know which developer agents exist — never hardcode agent names
