...
Developer agents are discovered at runtime by listing `.opencode/agents/`. Any file whose name ends in `-developer.md` is a developer agent. Common names:
...
Use the developer agents discovered at startup (from `.opencode/agents/`). Delegate to each relevant developer agent based on the plan:
